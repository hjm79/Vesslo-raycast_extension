import { Action, ActionPanel, Icon, List, showToast, Toast, open, Color } from "@raycast/api";
      ...
      >
         {!data ? (
            ...
            searchResults.map((result) => (
               <AppListItem key={result.app.id} app={result.app} matchedFields={result.matchedFields} />
            ))
         )}
      </List>
   );
...
function AppListItem({ app, matchedFields }: { app: VessloApp; matchedFields: string[] }) {
   const subtitle = [
      app.version,
      app.developer,
      ...app.tags.map((t) => `#${t}`),
   ...
   app.sources.forEach((source) => {
      const color = source === "Brew" ? Color.Orange : source === "App Store" ? Color.Blue : source === "Sparkle" ? Color.Green : Color.SecondaryText;
