...
export default function Updates() {
   ...
   const sortedApps = useMemo(() => {
      ...
      switch (sortBy) {
         ...
         case "developer":
            return apps.sort((a, b) => (a.developer ?? "").localeCompare(b.developer ?? ""));
         case "source":
         default:
   ...
   const sparkleApps = sortedApps.filter((app) => app.sources.includes("Sparkle") && !app.sources.includes("Brew"));
   const appStoreApps = sortedApps.filter((app) => app.sources.includes("App Store"));
   const otherApps = sortedApps.filter(
      (app) => !app.sources.includes("Brew") && !app.sources.includes("Sparkle") && !app.sources.includes("App Store")
   );
   ...
   return (
      ...
      >
         {!data ? (
            ...
            <List.Section title={`Updates (${sortedApps.length}) - ${sortLabels[sortBy]}`}>
               {sortedApps.map((app) => (
                  <UpdateListItem key={app.id} app={app} />
               ))}
            </List.Section>
...
async function runBrewUpgrade(caskName: string, appName: string) {
   try {
      ...
      const { stdout, stderr } = await execAsync(`/opt/homebrew/bin/brew upgrade --cask ${caskName}`);
