import { Action, ActionPanel, Icon, List, showToast, Toast, Color, confirmAlert, Alert } from "@raycast/api";
...
export default function BulkHomebrewUpdate() {
   ...
   const homebrewAppsWithUpdates = useMemo(() => {
      ...
      return data.apps.filter(
         (app) => app.sources.includes("Brew") && app.targetVersion !== null && app.homebrewCask
      );
   }, [data]);
   ...
   async function updateAll() {
      ...
      try {
         ...
         const { stdout } = await execAsync("/opt/homebrew/bin/brew upgrade --cask");
   ...
   async function updateSingle(caskName: string, appName: string) {
      try {
         ...
         const { stdout } = await execAsync(`/opt/homebrew/bin/brew upgrade --cask ${caskName}`);
   ...
   return (
      <List isLoading={isLoading || isUpdating}>
         {homebrewAppsWithUpdates.length === 0 ? (
            ...
            <>
               <List.Section title={`Homebrew Updates (${homebrewAppsWithUpdates.length})`}>
                  {/* Update All item */}
                  <List.Item
